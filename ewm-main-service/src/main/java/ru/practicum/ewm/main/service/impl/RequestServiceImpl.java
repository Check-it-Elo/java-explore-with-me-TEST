...
public class RequestServiceImpl implements ru.practicum.ewm.main.service.RequestService {
    ...
    public ParticipationRequestDto addRequest(long userId, long eventId) {
        ...
        RequestStatus status = (noModeration || noLimit)
    ...
    public EventRequestStatusUpdateResult updateEventRequests(long userId, long eventId, EventRequestStatusUpdateRequest body) {
        ...
        if (!"CONFIRMED".equalsIgnoreCase(next) && !"REJECTED".equalsIgnoreCase(next)) {
            throw new ConflictException("Unknown status: " + next);
        }
        ...
        for (Long id : ids) {
            ParticipationRequest pr = requestRepository.findById(id)
                    .orElseThrow(() -> new NotFoundException("Request with id=" + id + " not found"));
            if (!pr.getEvent().getId().equals(eventId)) {
                throw new NotFoundException("Request with id=" + id + " not found for event=" + eventId);
            }
            if (pr.getStatus() != RequestStatus.PENDING) {
                // спецификация: менять можно только PENDING
                continue;
            }
            ...
            if ("CONFIRMED".equalsIgnoreCase(next)) {
                if (limit > 0 && confirmed >= limit) {
                    ...
                    rejectedDtos.add(requestMapper.toDto(requestRepository.save(pr)));
                } else {
                    pr.setStatus(RequestStatus.CONFIRMED);
                    confirmed++;
                    confirmedDtos.add(requestMapper.toDto(requestRepository.save(pr)));
                }
            } else { // REJECTED
                pr.setStatus(RequestStatus.REJECTED);
                rejectedDtos.add(requestMapper.toDto(requestRepository.save(pr)));
        ...
        // авто-отклонение остатка PENDING при закрытом лимите
        if (limit > 0 && confirmed >= limit) {
